...
- (void)imagebut:(UIButton *)sender {
    ...
    [UIView transitionWithView:sender
        ...
        if (sender.tag==1) {
            [sender setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"1"]] forState:UIControlStateNormal];
        }else{
            self.coinsint=self.coinsint+10;
            self.coslab.text=[NSString stringWithFormat:@"%d",self.coinsint];
            [sender setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"2"]] forState:UIControlStateNormal];
            NSString *tis=@"";
