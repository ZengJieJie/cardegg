//
//  SimplicityAdditionViewController.swift
//  Lckukynumber
//
...
import UIKit
import Masonry
class SimplicityAdditionViewController: LckukynumberBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.bRandomaray = bRandomarayRandomNumbers(count: 25, min: 1, max: 20)
        self.luckynumber.text="Lucky:\(self.LckyNumber) \(self.LckyNumberjian)"
        self.marklab.text="\(self.marksco)"
        createGrid()
    ...
    @IBAction func restartbtn(_ sender: Any) {
        ...
        let okAction = UIAlertAction(title: "OK", style: .default) { _ in
            ...
            self.marksco = 0
            self.timernumber = 60
            self.LckyNumber = self.RandomgenerationLckyNumber(between: 7, and: 18)
            self.LckyNumberjian = self.RandomgenerationLckyNumber(between: 2, and: 9)
            self.bRandomaray = self.bRandomarayRandomNumbers(count: 25, min: 1, max: 10)
            self.luckynumber.text="Lucky:\(self.LckyNumber) \(self.LckyNumberjian)"
    ...
    @objc func buttonClicked(_ sender: UIButton) {
        ...
        if let lastButton = lastNIBButton, let secondButton = secondNIBButton {
            if let ac = bRandomaray[lastButton.tag] as? NSNumber, let bc = bRandomaray[secondButton.tag] as? NSNumber {
                ...
                if (acbcSum == LckyNumber || acbcjian == LckyNumber||acbcSum == LckyNumberjian || acbcjian == LckyNumberjian) {
